...
def group(df):
    ...
    return (df_frecuencias,df_absolutas)

def bias(df):
    bias = Bias()
    bdf = bias.get_disparity_major_group(df=xtab, original_df=df, alpha=0.05)
    df_disparidad_sucio = bdf[['attribute_name', 'attribute_value'] + bias.list_disparities(bdf)].round(2)
...
def bias_fairness(test_path,model_path,model_output_df_path,
    ...
    model_output_df = load_df(model_output_df_path)
    ...
    df['delegacion'] = df_test.delegacion_inicio

    df_group_frecuencias,df_group_absolutas = group(df)
    df_disparidad_limpio = bias(df)
    fairness_result,fairness_gaf,fairness_gof = fairness(df)
